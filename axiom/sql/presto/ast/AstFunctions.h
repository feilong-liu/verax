...
class WhenClause : public Node {
 public:
  WhenClause(const ExpressionPtr& operand, const ExpressionPtr& result)
...
class CurrentUser : public Expression {
 public:
  CurrentUser(NodeLocation location)
...
class LambdaArgumentDeclaration : public Node {
 public:
  explicit LambdaArgumentDeclaration(const std::shared_ptr<Identifier>& name)
